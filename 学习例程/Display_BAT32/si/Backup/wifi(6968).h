...
{	//1-16
	...
	uint8_t MnrFilter3ResetTimes;		// 吸附滤重置次数  0-100   byte65

	uint8_t resever1;					// 预留   byte66
	uint8_t resever2;					// 预留 
	uint8_t resever3;					// 预留 
	uint8_t resever4;					// 预留 
	uint8_t resever5;					// 预留 // 70
	uint8_t resever6;					// 预留 
	uint8_t resever7;					// 预留 
...
{	//1-16
	...
	uint16_t rawTds;//进水TDS
	uint16_t pureTds;//出水TDS
	uint16_t thickFilmTemperature;//预留，厚膜温度
	uint16_t iceCanTemperature;//预留，冰罐温度
	//uint16_t rawToc;//预留，
	//uint16_t pureToc;//预留，
	//uint16_t rawUV254;//预留，
	//uint16_t pureUV254;//预留，
