...
void MonitorWater(void)
{
	...
	if(time.Minute!=min)
	{
		...
		if(gReg.WorkState==WORKSTATE_working)
		{
			if(systemseting.pcb_rest_flag){if(++systemseting.pcb_rest_1hcnt>=60){systemseting.pcb_rest_flag=0;}}
			else {gReg.PumpWorkeTimeTotal_PCB++;}

			if(systemseting.pcb_rest_flag){if(++systemseting.ro_rest_1hcnt>=60){systemseting.ro_rest_flag=0;}}
