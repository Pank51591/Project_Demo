...
//	INT8U *p;
	...
	if(crc==save->crc)
	{
		...
		systemseting.pcb_rest_flag = save->pcb_rest_flag;
		systemseting.ro_rest_flag = save->ro_rest_flag;

		gReg.fruits = save->fruits;
		gReg.BackFlow = save->BackFlow;

		gReg.error.u16flag = 0;
		gReg.error.BitFlag.sparklingWaterStatus = ((gReg.fruits)?1:0);
...
void SaveSysSeting(void)
{
	...
	save->pcb_rest_flag = systemseting.pcb_rest_flag;
	save->ro_rest_flag = systemseting.ro_rest_flag;
