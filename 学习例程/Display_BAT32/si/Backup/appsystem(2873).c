...
//	INT16U FilterLife;
	...
	if((remain>(720*60))&&(makingRemained>(2.4*60)))//720h-2.4h 
	{
		remain = PCB_level_3;
	}
...
void ExitWorkMode(void)
{
	...
	SetInletValve(RESET);//关闭进水阀
	
	TimeSetEvent(SYS_TIMERPUMP, PUMP_open_delaytime, ClosePump, TIME_ONESHOT);//延时200ms开启水泵
}
...
void MonitorHVS_Faucet(void)
{
	...
	if(gReg.WorkState==WORKSTATE_live)
	{
		...
		if(systemseting.FlowMeterBeforeVelocity>=1000)//打开电解片
		{
			...
			{
				opencount++;
				if(systemseting.fruits)//蔬果开启的时候才打开电解膜
				{
...
void ParsingDisplayData(INT8U *pbuff,INT16U len)
{
	...
	GetCommData(pbuff,len);//去掉数据中FF数据后面的55
...
{
	if(++systemseting.WinTimerCount<10)return;
...
void LEDLight(void)
{
	...
	I2C_SendDisplay((INT8U*)&gDisplayReg,sizeof(DISPLAY_REG));
