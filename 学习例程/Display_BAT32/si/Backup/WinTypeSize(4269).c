...
static void Update(void)
{
	...
	if(item==ITEM_1)
	{
		...
		{ 
			...
			LcdWriteStrimg(2,0,"                    ");
			snprintf(buf, 20, "%d          ", gUISet.Type[type].Size_Pulses[0]);
	...
	else if(item==ITEM_2)
	{
		...
		else if(focus==FOCUS_1)
		{
			...
			LcdWriteStrimg(2,0,"                    ");
			snprintf(buf, 20, "%d          ", gUISet.Type[type].Size_Pulses[1]);
			LcdWriteStrimg(3,0,buf);		
		}
	}
	else if(item==ITEM_3)
	{
		...
		else if(focus==FOCUS_1)
		{
			...
			LcdWriteStrimg(2,0,"                    ");
			snprintf(buf, 20, "%d          ", gUISet.Type[type].Size_Pulses[2]);
			LcdWriteStrimg(3,0,buf);
		}		
	}
	else if(item==ITEM_4)
	{
		...
		else if(focus==FOCUS_1)
		{
			...
			else 
			{
				...
				LcdWriteStrimg(2,0,"                    ");
				snprintf(buf, 20, "%d          ", gUISet.Type[type].Size_Pulses[3]);
				LcdWriteStrimg(3,0,buf);				
			}
		}		
	}	
	...
	case VK_DOWN:	
		{
			...
			{
				temp=gUISet.Type[type].Size_Pulses[0];
				...
				gUISet.Type[type].Size_Pulses[0]=temp;
			}
			else if(item==ITEM_2)
			{
				temp=gUISet.Type[type].Size_Pulses[1];
				...
				gUISet.Type[type].Size_Pulses[1]=temp;
			}
			else if(item==ITEM_3)
			{
				temp=gUISet.Type[type].Size_Pulses[2];
				...
				gUISet.Type[type].Size_Pulses[2]=temp;
			}
			else if(item==ITEM_4)
			{
				temp=gUISet.Type[type].Size_Pulses[3];
				...
				gUISet.Type[type].Size_Pulses[3]=temp;
			}			
		}
		break;
	case VK_SELECT:	
