...
void RestoreFactory(void)
{
	...
	systemseting.pcb_rest_flag = 1;//上电默认不计数
	systemseting.ro_rest_flag = 1;
...
void FilterReset_PCB(void)
{
	...
	systemseting.pcb_rest_flag=1;
	systemseting.pcb_rest_1hcnt=0;
...
void MonitorWater(void)
{
	...
	if(time.Minute!=min)
	{
		...
		if(gReg.WorkState==WORKSTATE_working)
		{
			if(systemseting.pcb_rest_flag){if(++systemseting.pcb_rest_1hcnt>=60){systemseting.pcb_rest_flag=0;SaveSysSeting();}}
			else {gReg.PumpWorkeTimeTotal_PCB++;}

			if(systemseting.pcb_rest_flag){if(++systemseting.ro_rest_1hcnt>=60){systemseting.ro_rest_flag=0;SaveSysSeting();}}
			else {gReg.PumpWorkeTimeTotal_RO++;}
...
void CheckError(void)
{
	...
	||systemseting.uartWifiError)
	{
		if(gReg.WorkState==WORKSTATE_standby)
		{
			gReg.WorkState=WORKSTATE_free;//有错误不进入睡眠
		}
	}
}
...
void MonitorState(void)
{
	...
	else if(GetHVSstate())//制水检测
	{
		if(systemseting.HVS==0)//进入制水状态
		{
			...
			systemseting.BackflowFlag=0;//退出回流重新计时
		}
		else 
		{
