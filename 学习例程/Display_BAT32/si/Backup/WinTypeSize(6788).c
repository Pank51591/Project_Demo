...
INT16U Gram2Pulses(INT16U gram)
{
	INT32U temp;

	temp=(4931*gram-68713+500)/1000;
	
	return temp; 
}

INT16U Pulses2Gram(INT16U Pulses)
{
	INT32U temp;

	temp=(((Pulses*10000-5000+687130)/4931)+5)/10;
...
static void Update(void)
{
	...
	if(item==ITEM_1)
	{
		...
		{ 
			...
			LcdWriteStrimg(2,0,"                    ");
			snprintf(buf, 20, "%d          ", Pulses2Gram(gUISet.Type[type].Size_Pulses[0]));
	...
	else if(item==ITEM_2)
	{
		...
		else if(focus==FOCUS_1)
		{
			...
			LcdWriteStrimg(2,0,"                    ");
			snprintf(buf, 20, "%d          ", Pulses2Gram(gUISet.Type[type].Size_Pulses[1]));
			LcdWriteStrimg(3,0,buf);		
		}
	}
	else if(item==ITEM_3)
	{
		...
		else if(focus==FOCUS_1)
		{
			...
			LcdWriteStrimg(2,0,"                    ");
			snprintf(buf, 20, "%d          ", Pulses2Gram(gUISet.Type[type].Size_Pulses[2]));
			LcdWriteStrimg(3,0,buf);
		}		
	}
	else if(item==ITEM_4)
	{
		...
		else if(focus==FOCUS_1)
		{
			...
			else 
			{
				...
				LcdWriteStrimg(2,0,"                    ");
				snprintf(buf, 20, "%d          ", Pulses2Gram(gUISet.Type[type].Size_Pulses[3]));
				LcdWriteStrimg(3,0,buf);				
			}
		}		
	}	
	...
	case VK_DOWN:	
		{
			...
			{
				temp=Pulses2Gram(gUISet.Type[type].Size_Pulses[0]);
				...
				gUISet.Type[type].Size_Pulses[0]=Gram2Pulses(temp);
			}
			else if(item==ITEM_2)
			{
				temp=Pulses2Gram(gUISet.Type[type].Size_Pulses[1]);
				...
				gUISet.Type[type].Size_Pulses[1]=Gram2Pulses(temp);
			}
			else if(item==ITEM_3)
			{
				temp=Pulses2Gram(gUISet.Type[type].Size_Pulses[2]);
				...
				gUISet.Type[type].Size_Pulses[2]=Gram2Pulses(temp);
			}
			else if(item==ITEM_4)
			{
				temp=Gram2Pulses(gUISet.Type[type].Size_Pulses[3]);
				...
				gUISet.Type[type].Size_Pulses[3]=Gram2Pulses(temp);
			}			
		}
		break;
	case VK_SELECT:	
