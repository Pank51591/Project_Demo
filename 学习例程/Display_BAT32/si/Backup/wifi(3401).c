...
#define TYPEID			"201461071080c014220f01904001020000000000000000000000000000000040"
#define DeviceKey		"900dffee988042Eda069c715A530df70"
#define VERSION			"E++2.17"
//#define SOFT_VERSION	"20211115"
#define HARD_VERSION	"20220523"
...
void MonitorBigData(void)
{
	if(gWifiSet.BigdataEN)
	{
		...
		if(++gWifiSet.BigdataHeartbeatCount>gWifiSet.BigdataHeartbeat)//心跳包
		{
			...
			EQUI_UpdataState_Bigdata(WIFI_CMD_06);
		}		
	}
}
...
void NetworkTimer(void)
{
	...
	if(systemseting.wifi==1)
	{
		systemseting.uartWifiError=0;
		KillTimer(0, SYS_TIMER_wifinetwork);
	}
	else if(networkcount>=600)//10分钟
	{
		systemseting.wifi=0;
		systemseting.uartWifiError=0;
		KillTimer(0, SYS_TIMER_wifinetwork);
	}
	else if(gWifiSet.workmode==WIFI_WORKMODE_work)
	{
		systemseting.uartWifiError=0;
		KillTimer(0, SYS_TIMER_wifinetwork);	
	}
...
void EQUI_EnterWifiNetwork(void)
{
	...
	EQUI_SendWifiNetwork(WIFI_WORKMODE_smartlink);
	TimeSetEvent(SYS_TIMER_wifinetwork, sec(3), NetworkTimer, TIME_PERIODIC);//定时3秒钟发一次
}
...
UN_ERROR_W GetAllError(ERRORFLAG_t flag)
{
	...
	error.BitFlag.outWaterTempSensorErrFlag = flag.TempErrorFlag;//温度传感器故障
	...
	error.BitFlag.timeoutFlag = flag.TimeOutErrorFlag;//制水超时
	...
	error.BitFlag.overheatErrFlag=0;
	error.BitFlag.overheatAlarmFlag=0;
...
void EQUI_UpdataState(INT8U cmd)
{
	...
	pupinfo.OutWaterTemp = gReg.rawTemperature&0x00FF;//进水温度

	pupinfo.priFilterRemain = gReg.FilterLife_PCB;
	pupinfo.roFilterRemain = gReg.FilterLife_RO;

	pupinfo.priFilterWarningFlag = (gReg.FilterLife_PCB<=10?1:0);//初滤到期预警(剩余10%),0无预警
	pupinfo.roFilterWarningFlag = (gReg.FilterLife_RO<=10?1:0);

	pupinfo.priFilterAlarmFlag = (gReg.FilterLife_PCB==0?1:0);//初滤到期报警（剩余0%)
	pupinfo.roFilterAlarmFlag = (gReg.FilterLife_RO==0?1:0);
...
//	pupinfo.roFilter2ResetTimes = gReg.RoFilterResetTimes;
	...
	SmallMode2BitMode(p+1+2-1,2);//pupinfo.rawTds
	SmallMode2BitMode(p+3+2-1,2);//pupinfo.pureTds

	SmallMode2BitMode(p+21+2-1,2);//pupinfo.perpureVolume
	SmallMode2BitMode(p+23+2-1,4);//pupinfo.pureVolume

	SmallMode2BitMode(p+27+2,4);//pupinfo.OutWaterTemp
	SmallMode2BitMode(p+43+2,2);//pupinfo.outWaterVolume

	SmallMode2BitMode(p+47+2,2);//pupinfo.freeWaterVolume
...
{
	UPStatuInfo_big pupinfo;
	SYSTEMTIME time;
	INT8U i,*p=(INT8U*)&pupinfo;
	static INT16U PriFilterResetTimes=0,roFilterResetTimes=0;

	memset(p,0,sizeof(UPStatuInfo_big));
	GetSystemTime(&time);
...
//	pupinfo.roFilter2ResetTimes = gReg.RoFilterResetTimes;
	...
	SmallMode2BitMode(p+21+2-1,2);//pupinfo.instantPurifiedWaterVolume

	SmallMode2BitMode(p+23+2-1,3);//pupinfo.pureVolume
