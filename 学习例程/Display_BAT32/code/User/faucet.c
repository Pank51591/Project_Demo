...
void Faucet_PowneOnDisplay(void)
{
	...
	if(systemseting.PowerOnTimerCount>6)//4秒
	{
	...
	else if(systemseting.PowerOnTimerCount==4)//3秒钟
	{
		faucet.QR=2;
	}
	else if(systemseting.PowerOnTimerCount==2)//2秒钟
	{
		faucet.QR=3;
	}
	else //1秒钟
...
void Faucet_Display(void)
{
	...
	if(faucet.workstate==FAUCET_workmode_error)
	{
...
/*static INT8U faucetgettimeoutcount=0;
void Faucet_UartErrorCheck(void)//500毫秒进来一次
{
	if(++faucetgettimeoutcount>10)//5秒没收到数据复位一次串口 处理静电打死的问题
	{
		faucetgettimeoutcount=0;

		//CGC->PER0 &= ~CGC_PER0_SCI0EN_Msk;//先关闭时钟
		//UART2_Init(SystemCoreClock, 9600);
		
		//CALL_ADDR(0);//5秒收不到数据软件复位
	}
}*/
...
void Faucet_proc(INT8U*pbuf,INT16U len)
{
	...
	if(pbuf[3]==0x81)//水龙头唤醒
	{
		...
		systemseting.uartFaucetTimeOutCnt=0;
	...
	systemseting.uartFaucetTimeOutTime=240;//120s
