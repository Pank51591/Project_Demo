...
static const char keyval[17][21]=
{
	"Shaken : null       ",
	"Blended : null      ",
	"Cold Foam : null    ",
...
static void Update(void)
{
	...
	if(item==ITEM_1)
	{
		...
		{
			if(gUISet.Service.MainButtonTestVal<=15)
			{
	...
	else if(item==ITEM_2)
	{
		...
		else if(focus==FOCUS_3)
		{
			...
			LcdWriteStrimg(1,0,"                    ");
			LcdWriteStrimg(2,0,"                    ");
			snprintf(buf, 20, "Status: %s        ", gUISet.Service.LowLevel?"Empty ":"Filled ");
			LcdWriteStrimg(3,0,buf);		
		}
		else if(focus==FOCUS_4)
		{
			LcdWriteStrimg(0,0,"Drip Tray Test      ");
			LcdWriteStrimg(1,0,"                    ");
			LcdWriteStrimg(2,0,"                    ");
