...
void EQUI_SendAlarmInfo(INT8U cmd)
{
	...
	buf[2] = 0;//gReg.error.u16flag>>8;

	if(gReg.error.BitFlag.TimeOutErrorFlag)
	{
		buf[3] = 0x10;
	}
	else 
...
{
	static INT16U old=0xff,count=0,updatacount=0,oldcrc=0xff;
	INT16U crc;
...
void WIFI_GetWifiStatus(INT8U cmd,INT8U *pbuf)
{
	...
	{
		if(gWifiSet.workmode!=WIFI_WORKMODE_work)
		{
...
void NetworkTimer(void)
{
	...
	if(systemseting.wifi==2||gWifiSet.workmode==WIFI_WORKMODE_work)
	{
		...
		gWifiSet.workmode=WIFI_WORKMODE_work;
...
UN_ERROR_W GetAllError(ERRORFLAG_t flag)
{
	...
	error.BitFlag.TempErrorFlag = 0;//flag.TempErrorFlag;//温度传感器故障
	error.BitFlag.rawTdsErrorFlag = 0;//flag.rawTdsErrorFlag;//原水TDS传感器故障
	error.BitFlag.pureTdsErrorFlag = 0;//flag.pureTdsErrorFlag;//纯水TDS传感器故障
	error.BitFlag.WaterLeakErrorFlag =0;//flag.WaterLeakErrorFlag;//漏水报警
	error.BitFlag.TimeOutErrorFlag = flag.TimeOutErrorFlag;//制水超时
	error.BitFlag.WasteWaterErrorFlag = 0;//flag.WasteWaterErrorFlag;//废水比阀故障
	error.BitFlag.InWaterErrorFlag = 0;//flag.InWaterErrorFlag;//进水阀故障
