...
#define TXD0_PORT_SETTING() do{ \
        PORT->P74CFG = 0x05;        /* allocate TXD0 to P51 */ \
        PORT->P7   |=  (1 << 4);    /* P51 output high level */ \
        PORT->PM7  &= ~(1 << 4);    /* P51 is used as TXD0 output */ \
        PORT->POM7 &= ~(1 << 4);    /* P51 is normal output mode */ \
        PORT->PMC7 &= ~(1 << 4);    /* P51 digital function */ \
}while(0)

/* ToDo: You can allocate the RXD0 to any desired pins with RXD0PCFG register */
#define RXD0_PORT_SETTING() do{ \
        PORT->RXD0PCFG = 0x14;      /* allocate RXD0 to P50 */ \
        PORT->PM3  |=  (1 << 1);    /* P50 is used as RXD0 input */ \
        PORT->PMC3 &= ~(1 << 1);    /* P50 digital function */ \
}while(0)
#else 
/* ToDo: You can allocate the TXD0 to any desired pins with PxxCFG register */
#define TXD0_PORT_SETTING() do{ \
        PORT->P31CFG = 0x05;        /* allocate TXD0 to P31 */ \
        PORT->P3   |=  (1 << 1);    /* P51 output high level */ \
        PORT->PM3  &= ~(1 << 1);    /* P51 is used as TXD0 output */ \
        PORT->POM3 &= ~(1 << 1);    /* P51 is normal output mode */ \
        PORT->PMC3 &= ~(1 << 1);    /* P51 digital function */ \
}while(0)

/* ToDo: You can allocate the RXD0 to any desired pins with RXD0PCFG register */
#define RXD0_PORT_SETTING() do{ \
        PORT->RXD0PCFG = 0x21;      /* allocate RXD0 to P74 */ \
        PORT->PM7  |=  (1 << 4);    /* P50 is used as RXD0 input */ \
        PORT->PMC7 &= ~(1 << 4);    /* P50 digital function */ \
}while(0)
