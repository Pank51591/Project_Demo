...
uint8_t Temp_write_buff[WRITE_BUFF_SIZE] = {0};
uint32_t Calculation_Read_checksum = 0;
uint32_t Calculation_Write_checksum = 0;
...
uint32_t Temp_write_HalfWord;
uint8_t Version_buff[3]; // Head  Checksum  Version
...
static void USB_HostMsdFatfsUserReadWrite(usb_host_msd_fatfs_instance_t *msdFatfsInstance)
{
    ...
    fatfsCode = f_open(&file, _T("1:/.bin"), FA_READ); // FA_WRITE | FA_READ
    ...
    fatfsCode = f_stat(_T("1:/.bin"), &fileInfo);
    if (fatfsCode)
    {
        //usb_echo("error\r\n");
        USB_HostMsdFatfsTestDone();
    ...
    Loop_count = 0;
    Calculation_Read_checksum = 0;

    //usb_echo("iap file f_seek......");
    fatfsCode = f_lseek(&file, 0);
    ...
    while (Remaining_size) // lopp calculation checksum
    {
        ...
        {
            Calculation_Read_checksum = Calculation_Read_checksum + Temp_read_buff[Temp_count];
        }
    ...
    //usb_echo("file Read Checksum= %08x \r\n", Calculation_Read_checksum);

    Version_buff[0] = (*((__IO u8 *)(BootJumpFlag)));     // 0x4D4D3332
    Version_buff[1] = (*((__IO u8 *)(BootJumpFlag + 2))); ////Checksum
    Version_buff[2] = (*((__IO u8 *)(BootJumpFlag + 4))); //

    //usb_echo("BootJumpFlag %02x %02x %02x \r\n", Version_buff[0], Version_buff[1], Version_buff[2]);

    if (Version_buff[0] != BOOT_HEAD)
    {
        if (Version_buff[1] != Calculation_Read_checksum)
        {

            //usb_echo("Start Erase application\r\n");

            Bootloader_Erase_APP(APP_SIZE, ApplicationAddress);

            //usb_echo("Erase success\r\n");

            // Write flash
            Remaining_size = fileInfo.fsize;
            Packet_Total_lenth = fileInfo.fsize;

            //usb_echo("iap get file \"Update.bin\" Transmission length = %d \r\n", Packet_Total_lenth);
            ...
            while (Remaining_size) // lopp calculation checksum
            {
                ...
                //usb_echo("Flash write one packet......");

                FLASH_Writeonepacket(Operation_address, Temp_read_buff, Updatepacketsize);
                Operation_address = Operation_address + Updatepacketsize;

                //usb_echo("write success\r\n");
...
void USB_HostMsdTask(void *arg,uint8_t param)
{
    usb_status_t status;
