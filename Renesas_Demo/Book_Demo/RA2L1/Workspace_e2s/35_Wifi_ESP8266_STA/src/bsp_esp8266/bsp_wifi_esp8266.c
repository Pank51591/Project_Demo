...
_Bool               Uart2_Receive_Flag = false; //用来判断UART2接收以及发送数据是否完成
...
void esp8266_uart2_callback(uart_callback_args_t * p_args)
{
       switch(p_args->event)
       {
           case UART_EVENT_RX_CHAR:
                ...
                At_Rx_Buff[Uart2_Num++] = ( char ) p_args->data;  //将UART2收到的数据放到Buff缓冲区中
           ...
           {
                Uart2_Receive_Flag = true;      //ESP8266回应完成标志
