 ...
 * @brief A-law algorithm for encoding and decoding (16bit pcm <=> a-law).  用于编码和解码的A-law算法（16bit pcm<=>A-law）。
...
#include <inttypes.h>  /// for appropriate size int types （对于适当大小的int类型）
...
/* result decoded for test 解码测试结果 */
...
void encode(uint8_t *out, int16_t *in, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++)
    {
         ...
         * same time  使用掩码同时定位第一个“1”位和量化级别*/
...
void decode(int16_t *out, uint8_t *in, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++)
    {
        ...
        /* Left shift according quantization level 根据量化级别左移*/
        pcm <<= eee > 1 ? (eee - 1) : 0;

        /* Use the right sign  使用正确的标志*/
        *out++ = sign ? -pcm : pcm;
    }
}
...
static void test(int16_t *pcm, uint8_t *coded, int16_t *decoded, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++)
    {
        assert(coded[i] == r_coded[i]);     //assert 的作用是先计算表达式 expression ，如果其值为假（即为0），那么它先向 stderr 打印一条出错信息,然后通过调用 abort 来终止程序运行。
    }
...
int main(int argc, char *argv[])
{
    ...
    test(pcm, coded, decoded, LEN);  // run self-test implementations  运行自检
