...
static void CAN_Mode_Config(void)
{
  ...
  RCC_APB1PeriphClockCmd(CAN_CLK, ENABLE);
	...
	CAN_InitStructure.CAN_Prescaler =4;		       //BTR-BRP 波特率分频器 (时间片的长度) 定义了时间单元的时间长度 36/(1+5+3)/4=1 Mbps
...
void CAN_Config(void)
{
  ...
  CAN_NVIC_Config();
  CAN_Mode_Config();
  CAN_Filter_Config();      //过滤器配置 
}


/**
  * @brief  初始化 Rx Message数据结构体
  * @param  RxMessage: 指向要初始化的数据结构体 （CAN Rx报文结构定义）
  * @retval None
  */
void Init_RxMes(CanRxMsg *RxMessage)
{
  ...
  RxMessage->DLC = 0;        //指定将接收的帧的长度。
  RxMessage->FMI = 0;        //过滤器匹配序号
	
  for (ubCounter = 0; ubCounter < 8; ubCounter++)
  {
    RxMessage->Data[ubCounter] = 0x00;       //要接送的数据
  }
}
...
{	  
  ...
  TxMessage->RTR=CAN_RTR_DATA;				 //发送的是数据  (远程发送请求)
  TxMessage->DLC=8;							       //数据长度为8字节   //指定将接收的帧的长度
